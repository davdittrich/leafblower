...
## Corpus Check
- 86 files · ~445,276 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 446 nodes · 606 edges · 24 communities detected
...
Cohesion: 0.06
Nodes (43): WU-3: Adaptive damping (geometric blend, auto-trigger), algBCD at C=0 (Sinkhorn Block Coordinate Descent), Algorithm Selection Benchmark Design (2026-04-20), Bayesian Level Set Estimation benchmark (GP surrogate), Bug: bcd_sweep clamps during IPF violating Sinkhorn invariant, Best-iterate tracking (W at minimum observed errRp), Bounded Convergence Fix Design (2026-04-18), Bregman Dykstra (multiplicative KL-space Dykstra) (+35 more)
...
Cohesion: 0.06
Nodes (33): Dykstra alternating projections, Bug: iEPPA RK_ERR_INFEAS never returned (pre-clean-code-4), Bug: n_bounds_clamped undercount (leafblower-kssd), Bug: W_best snapshot incorrect across homotopy levels, Bug: wrapper normalization after solver breaks bounds_mode=unit, Clean Code Fixes Round 2 Plan (2026-04-18), Clean Code Fixes Round 3 Plan (2026-04-19), Plan: Code Review Fixes — ieppa.cpp + configure (6 tasks) (+25 more)

### Community 4 - "Community 4"
Cohesion: 0.1
Nodes (28): GREG calibration (Deville-Sarnal 1992, Newton QP for chi2), Modified LDLT factorization (Gill-Murray diagonal perturbation), Normal equations (compute_normal_equations), Bug: sinkhorn Dykstra correction a[c] unbounded accumulation causing exp overflow, calib_linalg.hpp — shared normal equations kernel, Calibration Solvers Redesign spec (2026-04-25), Calibration Solvers Plan D: calib_linalg + method=greg (2026-04-26), method='chebyshev' — LP for L∞ calibration (+20 more)

### Community 5 - "Community 5"
Cohesion: 0.09
Nodes (26): Bug: int overflow in M_cell*10 > n*9 routing, Bug: raking post-clamp normalization violates bounds, calib_dispatch.hpp — shared metric/rule dispatch header, convergence API (metric + rule + tol), convergence metric: chi2, convergence metric: grake_norm (survey::calibrate-compatible), convergence metric: l1_weight (autumn/anesrake standard), convergence metric: max_err (L∞ marginal error) (+18 more)

### Community 6 - "Community 6"
Cohesion: 0.13
Nodes (14): CellTable data structure (sort-based cell deduplication), bits_needed(), build_cell_table(), pack_key_compute(), pack_key_fits(), ieppa_solve(), src/cell_table.hpp + cell_table.cpp, bits_needed() (+6 more)

### Community 7 - "Community 7"
Cohesion: 0.16
Nodes (18): Clean Code Fixes Round 1 Plan (2026-04-18), PYBIND11_MODULE(), Plan A Review Fixes (c_api field aliasing, Inf init), src/c_api.cpp — C API + algorithm selection, pack_lbfgsb_result(), pack_solver_result(), rk_calibrate(), rk_params_init() (+10 more)
...
Cohesion: 0.18
Nodes (15): Augmented Lagrangian Method (ALM for sum(w)=n in L-BFGS-B), Cell table (sort-based dedup, cell-compressed representation), Chebyshev calibration (true L-inf minimum), GRAKE calibration (normalized Chebyshev), iEPPA (Sinkhorn-BCD / algBCD calibration), L-BFGS-B with logit/exp dual link, Logarithmic barrier method (central-path, LP for L-inf), Raking (multiplicative iterative proportional fitting) (+7 more)

### Community 12 - "Community 12"
Cohesion: 0.14
Nodes (14): GP posterior mean (log10_compression=0): contour map of log(t_iEPPA/t_raking) over complexity x tol space; raking faster at high complexity, K-stability plot: 1.2x contour at K=3,9,18 showing robustness of GP algorithm selection boundary across GP ensemble size, GP posterior uncertainty: high posterior sd (orange/yellow) indicates unreliable algorithm selection regions in complexity x tol space, Plan: Calibration Solvers Plan B (Raking Cell-Table Migration), Cell-Table Backend (compressed obs-level representation), Descent Monitor (stall detection via kMaxNoImprove), Plan: Descent Monitor + kk1.20.4 Investigation (WU-P1, WU-P2), Gaussian Process emulator: models log(t_iEPPA/t_raking) surface over complexity x tol x compression space for algorithm selection (+6 more)
...
- **Thin community `Community 15`** (6 nodes): `lbw_math.hpp`, `bulk_exp_clipped()`, `bulk_log()`, `bulk_scaled_exp()`, `bulk_scaled_log()`, `lbw_math.hpp`
...
- **Thin community `Community 19`** (3 nodes): `conftest.py`, `Ensure the installed wheel's leafblower package is found, not the local source t`, `conftest.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 55`** (1 nodes): `src/leafblower.h — public C header + enum definitions`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `CellTable data structure (sort-based cell deduplication)` connect `Community 6` to `Community 1`, `Community 3`, `Community 4`, `Community 5`, `Community 7`, `Community 9`, `Community 10`?**
  _High betweenness centrality (0.203) - this node is a cross-community bridge._
- **Why does `harvest() calibration entry point` connect `Community 1` to `Community 0`, `Community 3`, `Community 5`, `Community 8`, `Community 12`?**
  _High betweenness centrality (0.162) - this node is a cross-community bridge._
- **Why does `method='ieppa' — paper-faithful algBCD solver` connect `Community 1` to `Community 9`, `Community 3`, `Community 5`, `Community 6`?**
