...
extern "C" SEXP cp_solve_R(
    ...
    if (Rf_length(A_p) != n_row + 1) Rf_error("length(A_csr$p) != n_row + 1");
    if (Rf_length(A_j) != Rf_length(A_x)) Rf_error("length(A_csr$j) != length(A_csr$x)");
...
extern "C" SEXP ipm_solve_R(
    ...
    IPMResult ipmres;
    if (infeasible) {
        ipmres.weights.assign(n_row, 1.0);
        ipmres.status_code = 3;
        ipmres.status_msg = "infeasible bounds";
        ipmres.iterations = 0;
        ipmres.wall_time_ms = 0.0;
    } else {
        ipmres = ipm_calibrate(
            n_row, n_col,
            INTEGER(A_p), INTEGER(A_j), REAL(A_x),
            REAL(b), REAL(d), p_lo, p_hi,
            max_iters, do_trace
    ...
    num_protects++;
    for (int i = 0; i < n_row; i++) {
        REAL(out_weights)[i] = ipmres.weights[i];
    }
    SET_VECTOR_ELT(result, 0, out_weights);
    ...
    PROTECT(out_status_code = Rf_allocVector(INTSXP, 1));
    num_protects++;
    INTEGER(out_status_code)[0] = ipmres.status_code;
    SET_VECTOR_ELT(result, 1, out_status_code);

    SEXP out_status_msg;
    PROTECT(out_status_msg = Rf_allocVector(STRSXP, 1));
    num_protects++;
    SET_STRING_ELT(out_status_msg, 0, Rf_mkChar(ipmres.status_msg.c_str()));
    SET_VECTOR_ELT(result, 2, out_status_msg);

    SEXP out_iters;
    PROTECT(out_iters = Rf_allocVector(INTSXP, 1));
    num_protects++;
    INTEGER(out_iters)[0] = ipmres.iterations;
    SET_VECTOR_ELT(result, 3, out_iters);

    SEXP out_wall_time;
    PROTECT(out_wall_time = Rf_allocVector(REALSXP, 1));
    num_protects++;
    REAL(out_wall_time)[0] = ipmres.wall_time_ms;
    SET_VECTOR_ELT(result, 4, out_wall_time);

    /* Trace: 8 columns per spec Sec 3.2 (iter_outer, iter_inner, time_ms, mu,
     * max_err, kkt_resid, n_projected_dims, alpha_step). Hard cap 1000 rows. */
    int num_cols = 8;
    int num_rows = (int)(ipmres.trace_data.size() / num_cols);
    if (num_rows > 1000) {
        Rf_error("trace exceeded 1000 rows limit");
    }
    SEXP out_trace;
    ...
    for (int r = 0; r < num_rows; r++) {
        for (int c = 0; c < num_cols; c++) {
            trace_ptr[c * num_rows + r] = ipmres.trace_data[r * num_cols + c];
        }
    }
    ...
    PROTECT(trace_colnames = Rf_allocVector(STRSXP, num_cols));
    num_protects++;
    SET_STRING_ELT(trace_colnames, 0, Rf_mkChar("iter_outer"));
    SET_STRING_ELT(trace_colnames, 1, Rf_mkChar("iter_inner"));
    SET_STRING_ELT(trace_colnames, 2, Rf_mkChar("time_ms"));
    SET_STRING_ELT(trace_colnames, 3, Rf_mkChar("mu"));
    SET_STRING_ELT(trace_colnames, 4, Rf_mkChar("max_err"));
    SET_STRING_ELT(trace_colnames, 5, Rf_mkChar("kkt_resid"));
    SET_STRING_ELT(trace_colnames, 6, Rf_mkChar("n_projected_dims"));
    SET_STRING_ELT(trace_colnames, 7, Rf_mkChar("alpha_step"));
