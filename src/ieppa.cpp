...
// Helper: post-loop finalization — obs expansion, bounds enforcement, best-iterate fallback.
// Called once per ieppa_solve() after the homotopy level loop exits.
// Moves ~284 lines out of ieppa_solve for readability; zero behavior change.
static void ieppa_finalize(
    CalibState&                               st,
    IEPPAResult&                              res,
    const CellTable&                          ct,
    std::vector<double>&                      X,
