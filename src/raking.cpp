...
//   cell_id == -1 → Stage-1: full-data raking on all margins.
...
RakingResult raking_solve_hierarchical(CalibState& st, const rk_params_t* p) {
    ...
    // cell_id == -1: full-data Stage-1 raking on ALL margins.
    ...
    auto fn = [&](std::vector<double>& weights_ref, int cell_id) {
        ...
        (void)weights_ref;
        if (cell_id < 0) {
            // Stage-1: full-data raking on all margins; mutates working_weights via st.weights.
            raking_solve(st);
    ...
    if (mode == 0) {
        ...
        // Stage-1: full-data raking on all margins.
        auto s1 = raking_solve(st);  // writes to working_weights via st.weights
