...
# Column 0 is the constant vector 1 (leafblower-xfz4: the calibration column space must
# span the constant, or u is not a GREG residual — see src/design_effect.cpp). Levels
# taken from target$region names (N S E W); code 0 = N dropped -> columns 1=intercept,
# 2=S, 3=E, 4=W. Tolerance 1e-11: measured relative difference between the package value
# and this hand-rolled oracle on this fixture is 1.974e-12 (R vs C++ float accumulation
# order); 1e-11 is the next power of ten above the measurement.
test_that("design_effect 4-arg matches hand-rolled Eq 3.5 oracle (rtol=1e-11, survey fallback)", {
  ...
  # Reproduce C++ X matrix: constant column + levels from target names, 0-based codes,
  # drop level 0.
  lvls <- names(target2$region)           # N S E W
  codes <- as.integer(factor(data2$region, levels = lvls)) - 1L  # 0=N,1=S,2=E,3=W
  p <- length(lvls)                      # 4 columns: intercept, S, E, W
  X2 <- matrix(0.0, nrow = n, ncol = p)
  X2[, 1L] <- 1.0                        # column 1: constant
  for (i in seq_len(n)) { c_i <- codes[i]; if (c_i >= 1L) X2[i, 1L + c_i] <- 1.0 }
  ...
  expect_equal(d_lbw2, d_analytic2, tolerance = 1e-11,
