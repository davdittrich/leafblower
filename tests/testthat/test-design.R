...
test_that("design_effect 4-arg 3-level perfect-fit -> deff_H = 0", {
  # K=1, 3 levels (A/B/C, 20 obs each), uniform w=1, y = group mean (perfect fit).
  # X = [1, ind_B, ind_C] (constant column + drop-first encoding, leafblower-xfz4).
  # beta_hat = [10, 10, 20] (intercept=10, B-slope=10, C-slope=20); with the constant in
  # the column space the calibration model reproduces every group mean exactly, so all
  # residuals u are 0 (verified: max|u| ~3.5e-15, floating-point zero).
  # var_u = 0; var_y = mean((y - y_bar)^2) = 200/3.
  # deff_H = deff_K * var_u / var_y = 1 * 0 = 0.
  n <- 60L
