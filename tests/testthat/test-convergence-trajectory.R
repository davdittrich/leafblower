# STUDY-BRANCH-ONLY-DO-NOT-MERGE: updated for leafblower-2ouc.48 dual-metric
# trajectory CSV schema (iter,<natural>,marginal_kl) — was iter,errRp only.
test_that("LBW_TRAJECTORY_AT env var produces probe file", {
  ...
  expect_named(probes, c("iter", "errRp", "marginal_kl"))
  ...
  expect_true(all(is.finite(probes$marginal_kl) & probes$marginal_kl >= 0))
})
