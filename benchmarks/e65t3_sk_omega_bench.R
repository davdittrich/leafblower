# e65t.3 benchmark: sinkhorn sk_omega sweep
# Tests omega in {1.0, 1.2, 1.4} x 4 configs x 3 seeds
# Verdict: GO if omega>1 reduces iterations >=10% with zero divergence/bracket failures
# Convergence metric: kl (sinkhorn's natural metric; harvest.R:368 maps sinkhorn->"kl")
library(leafblower)
...
# 4 configs per task spec e65t.3.4
# max_weight in {1.5, 2.0} exercises Dykstra projection path
configs <- list(
  list(name="K3_small",   n=200,  K=3, max_weight=3.0, min_weight=0.3),
  list(name="K6_medium",  n=500,  K=6, max_weight=2.0, min_weight=0.4),
  list(name="K9_bounded", n=800,  K=9, max_weight=1.5, min_weight=0.5),
...
# Verdict metrics
divergence_count  <- sum(res_df$diverged, na.rm=TRUE)
bracket_failures  <- sum(res_df$status == -1L, na.rm=TRUE)
...
cat(sprintf("\nDivergence count (INFEAS/error):   %d\n", divergence_count))
cat(sprintf("Bracket failures (status==-1):      %d\n", bracket_failures))
cat(sprintf("Median iter improvement omega=1.2: %.1f%%\n", med_imp_12))
cat(sprintf("Median iter improvement omega=1.4: %.1f%%\n", med_imp_14))

# GO criterion: zero divergence AND zero bracket failures AND >=10% improvement at 1.2 or 1.4
verdict <- if (divergence_count == 0 &&
               bracket_failures  == 0 &&
               (med_imp_12 >= 10 || med_imp_14 >= 10)) "GO" else "NO-GO"
cat(sprintf("\nVERDICT: %s\n", verdict))
