#!/usr/bin/env python3
"""
benchmarks/stepstone_benchmark.py

Real-world benchmark: Python leafblower
Companion to stepstone_benchmark.R — reads the parquet and JSON targets
produced by the R script, runs Python leafblower, and reports timing.

Usage:
    cd /path/to/leafblower
    Rscript benchmarks/stepstone_benchmark.R   # generates data
    python3 benchmarks/stepstone_benchmark.py  # this script
"""
...
DATA_PATH    = ROOT / "benchmarks" / "stepstone_bench_data.parquet"
TARGETS_PATH = ROOT / "benchmarks" / "stepstone_bench_targets.json"
...
def load_data():
    if not DATA_PATH.exists():
        raise FileNotFoundError(
            ...
            "Run: Rscript benchmarks/stepstone_benchmark.R first."
        )
    df = pd.read_parquet(DATA_PATH)
    with open(TARGETS_PATH) as f:
        targets_raw = json.load(f)
    # JSON targets: dict[margin_name -> dict[level -> proportion]]
    # Renormalise to exactly 1.0 (JSON float round-trip introduces ~1e-5 error)
    targets = {}
...
def bench_harvest(df: pd.DataFrame, targets: dict, n_runs: int = 5) -> dict:
    """Time leafblower.harvest() over n_runs; return timing + result stats."""
    from leafblower import harvest

    # Warmup (excludes JIT, paging, module import overhead)
    ...
    # Memory: peak allocation during one calibration call
    tracemalloc.start()
    harvest(df, targets, method=METHOD, max_weight=MAX_WEIGHT,
            max_iterations=MAX_ITER, convergence={"absolute": TOL},
            attach_weights=False)
...
def design_effect(weights: np.ndarray) -> float:
    """Kish (1965) design effect: n * sum(w²) / sum(w)²."""
    n = len(weights)
...
def main():
    print("=== Python leafblower — stepstone-style benchmark ===")
    ...
    print("--- Python leafblower::harvest ---")
    result = bench_harvest(df, targets)
    w = result["weights"]

    print(f"  time (median): {result['median_ms']:.0f} ms")
