#!/usr/bin/env Rscript
# T1: dump all 37 C_rk_calibrate args for chebyshev as R resolves them.
# Call: harvest(method='chebyshev', convergence=list(tol=1e-4),
...
# convergence=list(tol=1e-4) → tol overridden, metric=default→max_err(chebyshev), rule=improvement
conv <- leafblower:::parse_convergence(convergence_arg)

# Per-method metric override (harvest.R:308-331):
# chebyshev is NOT in the override map → no override → default from parse_convergence = max_err
# (R comment line 329: "chebyshev minimizes L-inf = max_err; logit has no natural KL")
# conv$metric stays as parse_convergence default: "max_err"
...
cat("=== R arg dump: harvest(method='chebyshev', convergence=list(tol=1e-4), max_weight=5, max_iterations=3000) ===\n\n")
