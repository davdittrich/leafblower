#!/usr/bin/env python3
"""
benchmarks/2apm/winner_bench.py
T3 (leafblower-2apm.3): multi-DGP chebyshev winner determination — Python side.
Run BEFORE winner_bench.R (sequential, single-thread BLAS).
Outputs: benchmarks/2apm/results_Py_<dgp>.csv + weights feather per DGP/rep.
...
APMDIR  = ROOT / "benchmarks" / "2apm"
WDIR    = APMDIR / "weights_Py"
...
DGPS = {
    ...
    "small_hicard": {
        "data":    ROOT / "benchmarks" / "yh0l" / "small_hicard_data.parquet",
        "targets": ROOT / "benchmarks" / "yh0l" / "small_hicard_targets.json",
    },
}
...
def main():
    ...
    for dgp_name, dgp_cfg in DGPS.items():
        ...
        try:
            ...
            out_path = APMDIR / f"results_Py_{dgp_name}.csv"
    ...
    if all_rows:
        out_path = APMDIR / "results_Py_all.csv"
        with open(out_path, "w", newline="") as f:
