#!/usr/bin/env python3
# benchmarks/2apm/parity_bench_st_py.py
# T2 (leafblower-2apm.2): single-thread chebyshev parity bench — Python side.
# Enforces OMP/OPENBLAS/MKL=1 via os.environ.setdefault BEFORE numpy import.
# Outputs to benchmarks/2apm/weights_Py_st/ + benchmarks/2apm/parity_bench_st_Py.csv

import os
os.environ.setdefault("OMP_NUM_THREADS", "1")
os.environ.setdefault("OPENBLAS_NUM_THREADS", "1")
os.environ.setdefault("MKL_NUM_THREADS", "1")
# Also force via assignment (setdefault only sets if absent; these override any parent)
...
repo = Path(__file__).resolve().parent.parent.parent  # benchmarks/2apm -> repo root
...
WDIR    = repo / "benchmarks/2apm/weights_Py_st"
OUT_CSV = repo / "benchmarks/2apm/parity_bench_st_Py.csv"
MAX_IT  = 3000
MAX_WT  = 5.0
TOL     = 1e-4
NREPS   = 3
...
def run(label, conv, **kwargs):
    ...
    for _ in range(NREPS):
        ...
        try:
            res = harvest(df, tgt, min_weight=0.0, max_weight=MAX_WT,
                          max_iterations=MAX_IT, convergence=conv, verbose=0,
                          attach_weights=False, **kwargs)
    ...
    return {
        ...
        "status":      _STATUS.get(ri.get("status", -1), str(ri.get("status"))),
        "iterations":  ri.get("iterations", -1),
...
# chebyshev only — T2 cause attribution
cfg = [
