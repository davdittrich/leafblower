#!/usr/bin/env Rscript
# benchmarks/yh0l/winner_bench.R
# T3: multi-DGP winner determination — R side
# Run AFTER winner_bench.py (memory: gotcha-openblas-thread-oversubscription).
# Outputs: benchmarks/yh0l/results_R_<dgp>.csv + weights feather per DGP/rep.
...
YDIR   <- file.path(ROOT, "benchmarks", "yh0l")
WDIR   <- file.path(YDIR, "weights_R")
...
dgps <- list(
  ...
  small_hicard = list(
    data_path    = file.path(YDIR, "small_hicard_data.parquet"),
    targets_path = file.path(YDIR, "small_hicard_targets.json")
  )
)
...
load_dgp <- function(dgp) {
  ...
  # Keep only target columns; set factor levels = target key order
...
run_dgp <- function(dgp_name, dgp) {
  ...
  for (rep in seq_len(NREPS)) {
    ...
    # Compute Tier-1 metrics from weights
    w <- as.double(res)
...
for (dgp_name in names(dgps)) {
  ...
  tryCatch({
    ...
    out_path <- file.path(YDIR, sprintf("results_R_%s.csv", dgp_name))
...
if (length(valid) > 0) {
  ...
  write.csv(agg, file.path(YDIR, "results_R_all.csv"), row.names = FALSE)
  cat(sprintf("\nAll R results saved to benchmarks/yh0l/results_R_all.csv\n"))
