...
**Epic:** leafblower-2ouc · **Status:** design v7 (v6 + user strict-separation constraint: single end-to-end timing axis, WU-0 dropped, benchmark code fully separate from leafblower) · **Date:** 2026-07-08
...
- **Mechanism:** language-split, adapter-normalised harness. Each competitor + leafblower method wrapped in a uniform adapter `run(problem) -> {weights_ref, iterations|NA, status, converged, error_message, wall_time, peak_rss}` (single **end-to-end** timing axis — raw-rows-in → weights-out, the groupby/cell/contingency build timed INSIDE the solve for every arm, §5). Metrics recomputed from returned weights by shared code refactored out of the existing `fit_metrics`/`compute_metrics`. **Primary comparison = work-precision (achieved accuracy vs cost)**; wall-time is secondary and always annotated with independently-recomputed accuracy.
- **Forbidden:** **modifying ANY leafblower source** (src/, r_bridge.cpp, pybind bindings, DESCRIPTION, NAMESPACE, R/, python/leafblower/) or reaching into non-public internal headers — ALL benchmark code lives STRICTLY under `benchmarks/study/` and calls leafblower only through its PUBLIC API (`harvest()`) (user constraint 2026-07-08; this killed the WU-0 solve-only entry point — see §5); cross-language timing comparison; renormalising weights after a solver's own finalize; comparing weights *across objective families* as error; trusting a solver's self-reported `converged`; per-solver hand-tuned tolerances; letting competitor deps enter DESCRIPTION/`pyproject` (would break the DoD gate); running any timed measurement before the spec+matrix+metrics are git-frozen.
...
- **RQ1 (speed):** work-precision (accuracy vs wall-time) primary; wall-time secondary. **Single end-to-end wall-time axis** — raw-rows-in → weights-out, the groupby/cell/contingency build timed INSIDE the solve for EVERY arm (leafblower's `harvest()` is called whole; survey/ipfn/etc. likewise) — so no arm gets an aggregation-hoist advantage. FFI overhead separated *for leafblower only* (§5). Threads ∈ {1,4}.
- **RQ2 (memory):** peak RSS.
...
- **Single end-to-end timing boundary (raw-rows-in → weights-out), applied identically to EVERY arm.** The groupby/contingency-table/indicator/cell construction is timed **inside** the solve for all solvers — leafblower's `harvest()` is called whole (its `build_cell_table` is inside the timer), and `survey`'s internal model-matrix build + the IPF family's contingency-table build likewise. No arm gets its aggregation hoisted out — this is the fairness-neutral boundary (symmetric, no home-field in either direction). **A leafblower solve-only axis is deliberately NOT measured** (reverses the earlier v6 two-axis plan): it would require a new lower-level entry point INTO leafblower, and the benchmark is constrained to be STRICTLY SEPARATE from leafblower — no source edits, public `harvest()` only (see Mechanism/Forbidden; user constraint 2026-07-08 killed WU-0). The `python_ipf_benchmark.py:66-77` asymmetry (a groupby timed inside one arm's solve but hoisted out of another's) is fixed by timing the groupby inside the solve for ALL arms uniformly. leafblower's FFI/serialization split is still measured separately as a leafblower-only diagnostic (below), which does not require any leafblower change.
...
- [ ] **Strict separation** (user constraint): ALL benchmark code under `benchmarks/study/`, calling leafblower only via public `harvest()`; ZERO edits to leafblower source — DoD `git diff` proves `src/`·`R/`·`python/leafblower/`·`DESCRIPTION`·`NAMESPACE` untouched. Single end-to-end timing axis (no leafblower solve-only entry point; WU-0 dropped).
...
- [ ] Harness runs CORE matrix; emits tidy results (**single `wall_time_s` end-to-end axis**) + per-(solver,problem,thread,build) weight store; canary reported as "behaves-as-expected (investigate if not)", not a hard pass gate.
- [ ] Reports: work-precision (single end-to-end axis) + Dolan–Moré profiles (family-stratified, INFEAS=DNF) + quality tables + agreement heatmaps (**vector-correlation for strictly-convex families only; minimax judged on achieved-L∞ objective agreement, Blocker G**).
...
**v6→v7** (user constraint 2026-07-08 — benchmark STRICTLY SEPARATE from leafblower, no core edits): the v6 Blocker-A "two co-headline timing axes" is REVERSED to a **single end-to-end axis** for every arm. The solve-only axis required a new lower-level leafblower entry point (WU-0), which strict separation forbids (leafblower's only public entry is `harvest(data, target, …)` — a data frame, no pre-coded group_ids). **WU-0 DROPPED** (ticket closed); leafblower is timed whole via public `harvest()` like every whole-unit competitor — symmetric, no home-field. Strict-separation rule added to Mechanism/Forbidden (all code under `benchmarks/study/`, public API only, zero leafblower source edits, DoD `git diff` gate). Contract/schema: `wall_time_e2e`+`wall_time_solve` collapse to a single `wall_time`/`wall_time_s`. Affected §1-RQ1, §5, §7, §8, §11 + tickets .1(closed)/.16(contract amended)/.9/.12/.13. (A prior WU-0 coder attempt had edited 8 leafblower core files; reverted to HEAD, rebuilt clean, R 1031/0 + Py 140/0.) All other v6 fixes (Blockers C/G/H, Gaps B/D/E/F/I) unchanged.
