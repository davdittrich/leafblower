...
- **Same fixed point** *(KL/IPF family)*: ORIS, Raking, Sinkhorn, Greenkhorn, and Newton-KL all converge to the maximum-entropy (KL) calibration solution when feasible — they are different *algorithms for the same optimum*. GREG (χ²), Logit (logit distance), and Chebyshev (minimax) target **different** optima.
...
|--------|------|-----------------|----------------------|-------|-----------------|---------------|------------------------|------------------|-----------------|
| **ORIS** | 1 | IPF / Sinkhorn + SOR | KL (I-divergence) | 1st | finalize (SOFT: ADMM penalty) | cheap | ✅ | metric improvement / plateau | Moderate |
...
- **Order**: 1st = first-order (linear convergence, cheap steps); 2nd = second-order (super-linear/quadratic near optimum, expensive steps). The 1st-order block (ORIS/Raking/Sinkhorn/Greenkhorn) trades iteration count for per-step cheapness; the 2nd-order block (Chebyshev/GREG/Newton-KL/Logit) trades per-step linear algebra for few iterations.
- **Objective families**:
  - **KL / maximum entropy** → ORIS, Raking, Sinkhorn, Greenkhorn, Newton-KL (positive weights "for free").
...
- **Bounds enforcement** is the sharpest design axis: *finalize-only* (ORIS, Newton-KL) → *inline projection* (Raking water-fill, Sinkhorn Dykstra, Greenkhorn clamp) → *active set* (GREG) → *parametrised away* (Logit logistic link) → *LP constraint* (Chebyshev).
- **Stateless ⇒ SRAA-able**: only the solvers whose sweep is a pure fixed-point map (no carried correction vectors) can be wrapped by SRAA-m Anderson acceleration: ORIS, Raking, Greenkhorn. Sinkhorn (Dykstra state), the Newton/IPM methods (iterate-dependent factorisations) cannot.
- **Over-relaxation is ORIS-only**: among the first-order block, successive over-relaxation (the power-law step `scale = ratio^ω`, `ω > 1`, with an adaptive ω) is a net win **only for ORIS**, whose stateless full sweep matches the regime where the `ω ∈ (0, 2)` convergence theory and the optimal-ω rate are proven [thibault2021overrelaxed, lehmann2022overrelaxation]. It does **not** carry over to the other two IPF-family solvers: **Sinkhorn**'s stateful Dykstra+bisection correction breaks the power-law ≡ log-SOR equivalence (the ORIS rate estimator misreads correction grind as slowness and over-shoots), and **Greenkhorn**'s greedy single-coordinate selection falls outside the proven full-sweep regime with no published theorem and no measured speed-up. Both therefore run the plain `ω = 1` step. Running Anderson (SRAA-m) and SOR together on ORIS is likewise no help — enabling Anderson disables SOR, since the two accelerators target the same fixed-point map and do not compound.

## Selection guidance (practical)

| If you need… | Prefer | Why |
|--------------|--------|-----|
| Robust default, severe skew | **ORIS** | Cheap, log-stable, converges where Newton/IPM stall. |
...
- **Moderate**: ORIS, Greenkhorn — base algorithm proven, but production embellishments (SOR auto-adaptation, α-damping, greedy tie-handling, Anderson) are empirically guarded rather than formally proven.
...
RK_ALG_ORIS       = 1   // IPF/Sinkhorn + SOR  (KL)
...
RK_ALG_ORIS_SOFT  = 8   // ORIS + ADMM soft capacity
