...
**Plan rev 4** (plan-review-gate round 3, cap reached): excluded `tests/testthat/_problems/` (testthat scratch) from the grep-clean gate; confirmed `fixtures/stepstone_reference_run.R` is edited in WU3 Step 2b. Gate escalated to user (3-iteration cap) — WU5 grep-clean gate is the binding completeness backstop.
...
| grep -vE '(^|/)tests/testthat/_problems/' \
