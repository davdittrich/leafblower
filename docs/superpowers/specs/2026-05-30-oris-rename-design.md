...
```
(`tests/testthat/_problems/` is testthat scratch — auto-saved failing-test snapshots, not run by the suite, regenerated; excluded, not edited. `graphify-out/` is a generated knowledge-graph artifact — regenerated by `graphify update .` after the rename, excluded from the gate, never hand-edited.)
