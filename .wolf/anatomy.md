...
> Auto-maintained by OpenWolf. Last scanned: 2026-04-27T12:36:40.587Z
...
- `LICENSE` — Project license (~16 tok)
- `NAMESPACE` (~71 tok)
- `NEWS.md` — leafblower (development) (~772 tok)
...
- `interactions.jsonl` (~29175 tok)
- `issues.jsonl` (~111587 tok)
- `last-touched` (~5 tok)
...
- `active-plan.md` — Active Plan (~3045 tok)

## .claude/

- `settings.json` (~661 tok)
- `settings.local.json` (~972 tok)
...
- `harvest.R` — ' Generate calibrated weights (drop-in for autumn::harvest) (~6174 tok)
...
- `gen_ieppa_kl_ref.R` — Generates tests/testthat/fixtures/ieppa_kl_reference_stepstone.rds (~370 tok)
...
- `cost.json` (~59 tok)
- `GRAPH_REPORT.md` — Graph Report - leafblower  (2026-04-27) (~4453 tok)
- `graph.html` — graphify - graphify-out/graph.html (~65300 tok)
- `graph.json` (~84512 tok)
...
- `effective_sample_size.Rd` (~103 tok)
- `get_current_miss.Rd` (~141 tok)
- `harvest.Rd` — Declares algorithm (~2005 tok)
- `weighted_pct.Rd` (~109 tok)
...
- `_bindings.cpp` — include <pybind11/pybind11.h> (~2326 tok)
- `_harvest.py` — harvest (~4792 tok)
- `test_python.py` — Tests: harvest_returns_copy, convergence_unknown_key_raises, min_weight_badarg_python, default_convergence_is_max_err_improvement + 3 more (~950 tok)

## src/

- `.gitignore` — Git ignore rules (~4 tok)
- `c_api.cpp` — include "leafblower.h" (~4624 tok)
- `c_api.o` (~5027 tok)
- `calib_dispatch.hpp` — pragma once (~1903 tok)
- `calib_linalg.cpp` — include "calib_linalg.hpp" (~1136 tok)
- `calib_linalg.hpp` — Compute N = A × diag(D) × Aᵀ where A is the (n_cats_total × M_cell) (~495 tok)
- `calib_linalg.o` (~902 tok)
- `calib_validate.cpp` — include "calib_validate.hpp" (~857 tok)
- `calib_validate.hpp` — Pre-entry validation for new cell-table solvers (sinkhorn, chebyshev, greg, grake). (~313 tok)
- `calib_validate.o` (~1015 tok)
- `cell_table.cpp` — include "cell_table.hpp" (~1566 tok)
- `cell_table.hpp` — pragma once (~360 tok)
- `cell_table.o` (~5813 tok)
- `chebyshev.cpp` — include "chebyshev.hpp" (~6982 tok)
- `chebyshev.hpp` — pragma once (~326 tok)
- `chebyshev.o` (~12748 tok)
- `grake.cpp` — include "grake.hpp" (~6 tok)
- `grake.hpp` — pragma once (~25 tok)
- `grake.o` (~250 tok)
- `greg.cpp` — include "greg.hpp" (~1645 tok)
- `greg.hpp` — pragma once (~333 tok)
- `greg.o` (~4446 tok)
- `ieppa.cpp` — include "lbw_config.h" (~17193 tok)
- `ieppa.hpp` — pragma once (~704 tok)
- `ieppa.o` (~17968 tok)
- `lbfgsb_solver.cpp` — include "lbw_config.h" (~8542 tok)
- `lbfgsb_solver.hpp` — pragma once (~380 tok)
- `lbfgsb_solver.o` (~13993 tok)
- `lbw_config.h` — ifndef LBW_CONFIG_H (~56 tok)
- `lbw_math.hpp` — pragma once (~260 tok)
- `leafblower.h` — ifndef LEAFBLOWER_H (~2602 tok)
- `logit.cpp` — include "logit.hpp" (~112 tok)
- `logit.hpp` — pragma once (~921 tok)
- `logit.o` (~250 tok)
- `Makevars` (~72 tok)
- `Makevars.bak` (~72 tok)
- `Makevars.in` (~80 tok)
- `r_bridge.cpp` — include "leafblower.h" (~8722 tok)
- `r_bridge.o` (~16875 tok)
- `raking.cpp` — raking.cpp — classical bounded raking via cyclic IPF + Dykstra projections. (~4374 tok)
- `raking.hpp` — pragma once (~363 tok)
- `raking.o` (~6523 tok)
- `sinkhorn.cpp` — include "sinkhorn.hpp" (~2422 tok)
- `sinkhorn.hpp` — pragma once (~252 tok)
- `sinkhorn.o` (~6519 tok)
- `types.hpp` — pragma once (~953 tok)
- `validation.hpp` — pragma once (~1203 tok)

## tasks/

- `prd-leafblower-core.md` — Leafblower Core — Product Requirements Document (~11847 tok)
...
- `test-calib-linalg.R` — Direct tests for LDLT factorization and compute_normal_equations (~1439 tok)
- `test-calibration-solvers.R` (~3885 tok)
- `test-cell-table.R` (~497 tok)
- `test-compare.R` (~495 tok)
- `test-compat.R` (~302 tok)
- `test-config-defaults.R` (~600 tok)
- `test-convergence-criteria.R` — Declares KL (~4058 tok)
- `test-convergence-trajectory.R` (~253 tok)
- `test-design.R` (~223 tok)
- `test-eta-schedule.R` (~402 tok)
- `test-harvest.R` (~1188 tok)
- `test-homotopy.R` (~533 tok)
- `test-ieppa-bounds-mode.R` (~2355 tok)
- `test-ieppa-faithful.R` (~3928 tok)
- `test-ieppa-nonuniform-d.R` (~447 tok)
- `test-ieppa-persistent-infeas.R` — WU-1: persistent-infeas tracker regression test. (~1168 tok)
- `test-ieppa.R` (~658 tok)
- `test-lbfgsb.R` (~1084 tok)
- `test-logit.R` (~308 tok)
- `test-priority-sweep.R` (~783 tok)
- `test-quality-metrics.R` (~1266 tok)
- `test-raking.R` — Declares split (~1087 tok)
- `test-sor.R` (~459 tok)

## tests/testthat/fixtures/

- `ieppa_kl_reference_stepstone.rds` (~47 tok)
- `raking_obs_reference_stepstone.rds` (~56 tok)
