...
cat("Running ORIS on stepstone-fulldata to record KL at best_iter...\n")
suppressWarnings(
  w <- leafblower::harvest(data, target, method = "oris",
...
ref <- list(
  ...
  oris_version   = as.character(packageVersion("leafblower")),
  date            = Sys.Date()
)
cat(sprintf("  KL at best_iter=%d: %.4e\n", ref$best_iter, ref$kl_at_best_iter))
cat(sprintf("  max_error: %.4e\n", ref$max_error))

saveRDS(ref, "tests/testthat/fixtures/oris_kl_reference_stepstone.rds")
cat("Saved to tests/testthat/fixtures/oris_kl_reference_stepstone.rds\n")
